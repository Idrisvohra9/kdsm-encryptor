...
const X = {
  count: 200,
  // Updated colors array with the specified colors: white, grey, violet, blue, purple, black
  colors: [0xffffff, 0x808080, 0x8a2be2, 0x0000ff, 0x800080, 0x000000],
...
const BallPit = ({ className = '', followCursor = true, colors, ...props }) => {
  ...
  // Memoize the color configuration to prevent unnecessary re-renders
  const ballPitColors = useMemo(() => {
    return colors || [0xffffff, 0x808080, 0x8a2be2, 0x0000ff, 0x800080, 0x000000];
  }, [colors]);
  ...
  useEffect(() => {
    ...
    // Create ballpit instance with the specified colors
  ...
  }, [followCursor, ballPitColors, props]);

  return (
    <canvas
      className={`${className} w-screen h-[200vh] fixed top-0 left-0 z-[-1]`}
      ref={canvasRef}
    />
  );
};
